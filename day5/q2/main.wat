(module
	...
	(block $instLoopDone
		...
		;; push to aux stack first (use aux stack as stack+1)
		(local.set $i (i32.const 0))
		(loop $innerInstLoop1

		    (call $stackPush (local.get $stacks) (call $stackTop (local.get $from)))
		    (call $stackPop (local.get $from))

		    (local.set $i (i32.add (local.get $i) (i32.const 1)))
		    (br_if $innerInstLoop1 (i32.lt_u (local.get $i) (local.get $moves)))
		)
		(local.set $i (i32.const 0))
		(loop $innerInstLoop2

		    (call $stackPush (local.get $to) (call $stackTop (local.get $stacks)))
		    (call $stackPop (local.get $stacks))

		    (local.set $i (i32.add (local.get $i) (i32.const 1)))
		    (br_if $innerInstLoop2 (i32.lt_u (local.get $i) (local.get $moves)))
		)
		(br $instLoop)
